    ...
    <div className="flex items-center justify-between">
      ...
      >
        ...
        <nav className="hidden md:flex items-center space-x-6">
          <Link href="/features" className="text-gray-700 hover:text-gray-900 font-medium text-sm transition-colors hover:bg-[#EEFBFF] rounded-full px-4 py-2 mr-0">
            Features
          </Link>
          <Link href="/pricing" className="text-gray-700 hover:text-gray-900 font-medium text-sm transition-colors hover:bg-[#EEFBFF] rounded-full px-4 py-2 mr-3">
            Pricing
          </Link>
            ...
            {hoveredDropdown === 'company' && (
              <div className="absolute top-full left-0 mt-5 w-96 p-2 rounded-xl shadow-lg border border-gray-100 bg-white space-y-1 z-50 animate-fade-in">
                {COMPANY_MENU.map((item) => (
                  <Link key={item.href} href={item.href} className={menuItemClasses(item.href)} onClick={() => { setActivePath(item.href); setHoveredDropdown(null); }}>
      ...
      <div className={`flex items-center ${bgClass} rounded-full shadow-[0_3px_6px_rgba(181,181,181,0.25)] px-1 py-1 md:px-2 md:py-2 space-x-1 sm:space-x-4`} style={stickyStyle}>
        <Link href="/login" prefetch={false} className="text-gray-600 hover:text-gray-900 font-medium text-sm transition-colors ml-5 px-3 hidden md:block">
          Log In
        </Link>
        ...
        <div className="block md:hidden relative z-[60]">
          <Button type="button" variant="ghost" size="icon" className="rounded-full size-8 sm:size-10 touch-manipulation" aria-label="Open menu" aria-expanded={mobileMenuOpen} onClick={() => handleMobileMenuOpenChange(true)}>
            <svg className="h-6 w-6 sm:h-10 sm:w-10 text-gray-900" fill="none" viewBox="0 0 20 20" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
            </svg>
          </Button>

          {isMounted && mobileMenuOpen && (
            <div className="p-0 bg-transparent rounded-none border-0 shadow-none z-[200] pointer-events-auto duration-0 mobile-menu-sheet" style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 200 }}>
              <div className="mx-2 mt-2 rounded-[18px] border border-white/40 bg-white/90 shadow-[0_14px_36px_rgba(15,23,42,0.12)] overflow-y-auto relative flex flex-col max-h-[calc(100dvh-1.5rem)] touch-manipulation transform-gpu z-[1]">
                <div className="sticky top-0 z-[2] flex items-center justify-between px-4 pt-3 pb-2 bg-transparent shrink-0">
                  ...
                  <button type="button" aria-label="Close menu" className="relative z-[3] flex items-center justify-center size-10 opacity-90 rounded-full bg-white/45 shadow-md border border-white/45 touch-manipulation cursor-pointer" onClick={() => handleMobileMenuOpenChange(false)}>
                ...
                <nav className="pb-6 relative z-[1]">
                  {MOBILE_MENU_SINGLE_LINKS.map((item) => (
                    <Link key={item.href} href={item.href} prefetch={false} className="flex items-center gap-2 px-4 border-b border-[#93d8fa4c] py-3 min-h-[44px] text-gray-700 font-bold hover:bg-gray-100 transition-colors touch-manipulation relative z-[1]" onClick={(e) => { e.preventDefault(); const href = e.currentTarget.getAttribute('href'); handleMobileMenuOpenChange(false); if (href) router.push(href); }}>
                      <span>{item.label}</span>
                    </Link>
                  ))}

                  {MOBILE_MENU_GROUPS.map((group) => (
                    <Collapsible key={group.label} defaultOpen={false} open={isCompanyOpen} onOpenChange={setIsCompanyOpen}>
                      <CollapsibleTrigger type="button" className={cn('flex items-center px-4 gap-2 py-3 min-h-[44px] text-gray-700 font-bold hover:bg-gray-100 transition-colors w-full touch-manipulation relative z-[1]', isCompanyOpen ? '' : 'border-b border-[#93d8fa4c]') }>
                        <span>{group.label}</span>
                        <ChevronDown className={cn('size-4 ml-auto', isCompanyOpen ? 'rotate-180 transition-all ease-in-out duration-200' : '')} style={{ color: isCompanyOpen ? '#35B5F5' : undefined }} />
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                        <div className="flex flex-col w-full px-4 gap-1 pb-4 border-b border-[#93d8fa4c]">
                          {group.links.map((link) => (
                            <Link key={link.href} href={link.href} prefetch={false} className={cn('flex ml-2 items-start gap-2 py-2 px-1 min-h-[44px] text-gray-800 font-[400] transition-colors touch-manipulation relative z-[1]')} onClick={(e) => { e.preventDefault(); const href = e.currentTarget.getAttribute('href'); handleMobileMenuOpenChange(false); if (href) router.push(href); }}>
                              <div className="flex flex-col gap-[1px]"><span className="text-base leading-6">{link.label}</span></div>
                            </Link>
                          ))}
                        </div>
                      </CollapsibleContent>
                  ...
                  {MOBILE_MENU_RESOURCES.map((group) => (
                    <Collapsible key={group.label} defaultOpen={false} open={isResourcesOpen} onOpenChange={setIsResourcesOpen}>
                      <CollapsibleTrigger type="button" className={cn('flex items-center px-4 gap-2 py-3 min-h-[44px] text-gray-700 font-bold hover:bg-gray-100 transition-colors w-full touch-manipulation relative z-[1]', isResourcesOpen ? '' : 'border-b border-[#93d8fa4c]') }>
                        <span>{group.label}</span>
                        <ChevronDown className={cn('size-4 ml-auto', isResourcesOpen ? 'rotate-180 transition-all ease-in-out duration-200' : '')} style={{ color: isResourcesOpen ? '#35B5F5' : undefined }} />
                      </CollapsibleTrigger>
                      <CollapsibleContent>
                ...
                <div className="px-4 pb-6 pt-2 shrink-0 relative z-[1]">
                  <div className="w-full bg-white/40 rounded-full p-[5px] shadow-[0_14px_36px_rgba(15,23,42,0.14)] flex items-center justify-between gap-2 border border-white/45 will-change-auto">
                    <Link href="/login" prefetch={false} className="flex-1 text-gray-700 hover:text-gray-900 font-medium text-sm transition-colors px-3 py-3 min-h-[44px] rounded-full text-center bg-transparent touch-manipulation flex items-center justify-center" onClick={(e) => { e.preventDefault(); const href = e.currentTarget.getAttribute('href'); handleMobileMenuOpenChange(false); if (href) router.push(href); }}>
                      Log In
                    </Link>
                    <Button asChild className="flex-1 bg-gray-900/95 hover:bg-gray-800/90 text-white font-medium text-sm mx-0 px-4 h-[32px] rounded-full">
                      <Link href="/trial" prefetch={false} onClick={(e) => { e.preventDefault(); const href = e.currentTarget.getAttribute('href'); handleMobileMenuOpenChange(false); if (href) router.push(href); }}>
                        1-Month Free Trial
                      </Link>
                    </Button>
                  </div>
...
export function Header({ isHomePage = true }: { isHomePage?: boolean }) {
  ...
  const handleMobileMenuOpenChange = (open: boolean) => {
    // No freezing or lenis control — simply open/close
  ...
  // Immediate hover handlers — no delay
  const handleDropdownEnter = (dropdownName: string) => setHoveredDropdown(dropdownName);
  const handleDropdownLeave = () => setHoveredDropdown(null);

  return (
    <div className="w-full">
      <div id="gradient-bar" className="h-10 fixed top-0 left-0 w-full z-40 transition-opacity duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] pointer-events-none" style={{ background: 'linear-gradient(90deg, #6779FF 0%, #4E97FA 25%, #35B5F5 50%, #2EFFEA 100%)', opacity: !isScrolled ? 1 : 0, willChange: 'opacity' }} />

      <header className={`transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] ${isHomePage && !isScrolled ? ' header-vertical-lines' : ''} fixed left-0 w-full z-50 !bg-transparent`} style={{ maxWidth: '100vw', top: isScrolled ? '0px' : '0px', paddingTop: isScrolled ? '0px' : '0px', transform: isScrolled ? 'translateY(0)' : 'translateY(0)', willChange: 'transform, padding-top' }}>
        {!shouldHideGradient && (
          <div className="absolute inset-0 -z-10"><div className="absolute w-[300px] h-[150px] md:w-[500px] md:h-[300px] -top-[3rem] md:-top-[12rem] right-[5%] bg-gradient-to-r from-[#6779FF] to-[#4E97FA] opacity-30 blur-[10rem]" /></div>
        )}

        <div className="max-w-[73.1rem] mx-auto px-4 md:px-6 lg:px-4 py-6">
          <HeaderContent hoveredDropdown={hoveredDropdown} handleDropdownEnter={handleDropdownEnter} handleDropdownLeave={handleDropdownLeave} activePath={activePath} setActivePath={setActivePath} setHoveredDropdown={setHoveredDropdown} mobileMenuOpen={mobileMenuOpen} handleMobileMenuOpenChange={handleMobileMenuOpenChange} isSticky={isScrolled} />
        </div>
      </header>
